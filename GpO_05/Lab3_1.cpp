...
vec3 normal_T;
  ...
  vec3 pos_escena=(M*vec4(pos,1.0f)).xyz;
  ...
  ////////////////////////////
  vec3 r = reflect(-lightdir,normal_T);

  difusa=clamp(dot(lightdir,normal_T),0.0f,1.0f);
  
  float comp_esp = pow(clamp(dot(v,r),0.0f,1.0f),16);

  ilu=0.1+(0.6*difusa+0.3*comp_esp)*lightcolor;  //Ambiente + difusa
}
);
...
const char* vertex_prog2 = GLSL(       // PHONG
layout(location = 0) in vec3 pos;
layout(location = 1) in vec3 normal;
out vec3 normal_T;
out vec3 v;
...
void main()
{
	...
	vec3 nn=normalize(normal_T);

	vec3 r = reflect(-lightdir,nn);

  difusa=clamp(dot(lightdir,nn),0.0f,1.0f);
  
  float comp_esp = pow(clamp(dot(vv,r),0.0f,1.0f),relComp[3]);

  color_fragmento=relComp.x+(relComp.y*difusa+relComp.z*comp_esp)*lightcolor;  //Ambiente + difusa
...
void init_scene()
{
	...
	vec4 relComp = vec4(0.0f,0.0f,1.0f,16);
	transfer_vec4("relComp",relComp);
...
void keyboard(unsigned char key, int x, int y)
{
	...
	fprintf(stdout,"relComp (%3d,%3d,%3d,%3d)\n",xx,yy,zz,tt);
}
